...
**Authentication**: Optional (`OptionalUserDep`). Thread ID is derived from user ID for authenticated users (`user:{user_id}`) or from `session_id` cookie for guests. Sets `session_id` cookie for first-time anonymous users. In lesson mode, thread ID is `lesson:{user_or_session_id}:{lesson_id}`.
...
**Lesson mode**: When `lesson_id` is provided, the endpoint uses the lesson chat LangGraph (with phase machine) instead of the freeform chat graph. Level and language are derived server-side from the lesson metadata. On the first invocation (no existing checkpoint), the full lesson initialization state is sent. On subsequent turns, only the new user message is sent, preventing checkpoint state from being overwritten.
