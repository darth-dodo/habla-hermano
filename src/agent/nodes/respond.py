...
_STOPWORDS_PATH = Path(__file__).resolve().parents[3] / "data" / "stopwords.json"


@lru_cache(maxsize=1)
def _load_stopwords() -> frozenset[str]:
    """Load stopwords from data/stopwords.json and return as a frozen set.

    The result is cached so the file is read at most once per process.
    """
    with _STOPWORDS_PATH.open(encoding="utf-8") as fh:
        data = json.load(fh)
    words: set[str] = set()
    for key in ("english", "spanish"):
        words.update(data.get(key, []))
...
def _extract_keywords_from_messages(
    ...
    """
    # Load cached stopwords from data/stopwords.json
